...
import movieLinks from '../../data/links.json';
  ...
  useEffect(() => {
    ...
    const fetchMovie = async () => {
      ...
      const movieLink = movieLinks.find(
        (entry) => entry.id === id || entry.id === parseInt(id)
      );
      setLinks(movieLink);
    };
        ...
        <div className="flex flex-col gap-2">
          ...
          {links ? (
            <div className="mt-4">
              ...
              {links.streamLinks.map((link, i) => (
                <a
                  ...
                  className="inline-block bg-blue-500 text-white px-4 py-2 m-1 rounded"
                >
                  Server {i + 1} (Stream)
                </a>
              ))}
              {links.downloadLinks.map((link, i) => (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-block bg-green-500 text-white px-4 py-2 m-1 rounded"
                >
                  Download {i + 1}
                </a>
              ))}
            ...
            <p className="text-red-400 mt-4">
              No links available. Check back later.
            </p>
          )}
        </div>
      </div>
